...
from bj.models import GameSessionModel, StatusGameEnum, StatusPlayerEnum, PlayerModel
...
class BJGamePlay(BaseAccessor):
    ...
    def new_round(self, game_session_id: int) -> GameSessionModel:
    ...
    async def move_player(self, game_session_id: int, player_id: int, action: str) -> GameSessionModel:
        await self._get_game_session(game_session_id)
        await self._check_turn_player(player_id)
        if action == "move player" and self._check_deal_card():
            self.game_session.players[self.index].hand.append(self.game_session.deck.pop())
        self._check_round()
        return await self.update_game_session()

    async def quit_game_player(self, game_session_id: int, player_id: int) -> GameSessionModel:
        await self._get_game_session(game_session_id)
        for index, player in enumerate(self.game_session.players):
            if player.id == player_id:
                self.game_session.players.pop(index)
        return await self.update_game_session()

    async def round_end(self, game_session_id: int) -> GameSessionModel:
        await self._get_game_session(game_session_id)
        bot = self.game_session.players[-1]
        for player in self.game_session.players:
            if (bot.score < player.score < 22) or (player.score < 22 < bot.score):
                player.score += 2
            else:
                player.score -= 2
            player.balance += player.score
    ...
    async def _get_game_session(self, game_session_id: int):
        self.game_session = await self.app.store.bj.get_game_session_by_id(game_session_id)
    ...
    def _check_turn_player(self, player_id: int):
        if self.game_session.current_user_vk_id != player_id:
            raise ValueError(
                "it's not your turn, actions are expected from {}".format(self.game_session.current_user_vk_id))
    ...
    def _check_round(self):
        self.index += 1
        if self.index >= len(self.game_session.players):
            self.game_session.allowed_actions = ["round end"]
        else:
            self.game_session.current_user_vk_id = self.game_session.players[self.index].user_id

    def _check_deal_card(self) -> bool:
        return self.game_session.players[self.index].score < 21
